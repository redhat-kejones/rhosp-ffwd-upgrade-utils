...
def setup_nested_virt():
 ...
 if virt_extensions is "vmx":
  ...
  try:
   ...
   passthrough_get =  cmd(["crudini", "--get", "/var/lib/config-data/puppet-generated/nova_libvirt/etc/nova/nova.conf", "libvirt", "cpu_mode"])
  ...
  if not nova_passthrough_exists:
   ...
   print cmd(["crudini", "--set", "/var/lib/config-data/puppet-generated/nova_libvirt/etc/nova/nova.conf", "libvirt", "cpu_mode", "host-passthrough"])
   print "INFO: Restarting nova compute"
   #print cmd(["systemctl", "restart", "openstack-nova-compute.service"])
   print cmd(["docker", "restart", "nova_compute"])
